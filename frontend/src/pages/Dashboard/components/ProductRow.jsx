      ...
      {colunas.map((col) => {
        ...
        if (pref.regras && pref.regras.length > 0) {
          for (let regra of pref.regras) {
            ...
            const rawValue = produto[targetKey];
            
            const targetValue = (rawValue !== null && rawValue !== "" && !isNaN(rawValue)) ? Number(rawValue) : rawValue;
            const ruleValue = (regra.valor !== "" && !isNaN(regra.valor)) ? Number(regra.valor) : regra.valor;
            
            let match = false;

            if (typeof targetValue === 'number' && typeof ruleValue === 'number') {
            ...
            if (regra.operador === '=') {
                if (typeof targetValue === 'string' && typeof ruleValue === 'string') {
                    match = targetValue.toLowerCase() === ruleValue.toLowerCase();
                } else {
                    // eslint-disable-next-line eqeqeq
                    match = targetValue == ruleValue;
                }
            }
            ...
            if (match) {
              ...
              if (regra.size) activeSize = regra.size;
              break; 
            }
          }
        }
        ...
        switch (col.type) {
          ...
          case 'editable-currency': return <td key={col.key} style={customStyle} className={`py-1 px-1 text-center ${!activeText && 'text-zinc-300'}`}><EditableCell className={`${inputEstilo} text-center`} value={val} colunaKey={col.key} onChange={(newVal) => onCellEdit(index, col.key, newVal)} /></td>;
          case 'editable-currency-highlight': return <td key={col.key} style={customStyle} className={`py-1 px-1 text-center font-medium ${!activeText && 'text-blue-400'}`}><EditableCell className={`${inputEstilo} text-center text-blue-400`} value={val} colunaKey={col.key} onChange={(newVal) => onCellEdit(index, col.key, newVal)} /></td>;
          case 'editable-percent-1': return <td key={col.key} style={customStyle} className={`py-1 px-1 text-center ${!activeText && 'text-zinc-400'}`}><EditableCell className={`${inputEstilo} text-center`} value={val} colunaKey={col.key} isPercentage={true} decimals={1} onChange={(newVal) => onCellEdit(index, col.key, newVal)} /></td>;
