...
export default function ModalPesquisa({ isOpen, onClose, onSelect, ambiente }) {
  ...
  const handleFiltrar = async () => {
    ...
    try {
      const res = await api.get('/precificacao/pesquisar', {
  ...
  const buscarFornecedores = async (termo = "") => {
    try {
      const res = await api.get(`/precificacao/fornecedores?termo=${termo}&ambiente=${ambiente}`);
  ...
  const buscarClassificacoes = async () => {
    try {
      const res = await api.get(`/precificacao/classificacoes?ambiente=${ambiente}`);
